...
electron__WEBPACK_IMPORTED_MODULE_2__["app"].on("ready", () => {
  setApplicationMenu();
  electron__WEBPACK_IMPORTED_MODULE_2__["app"].setAsDefaultProtocolClient('faveslist');
  electron__WEBPACK_IMPORTED_MODULE_2__["app"].requestSingleInstanceLock();
  config = __webpack_require__(/*! ./services/config */ "./src/services/config.js");
  electron__WEBPACK_IMPORTED_MODULE_2__["globalShortcut"].register('CommandOrControl+Shift+K', () => {
    mainWindow.webContents.send('shortcut');
  });

  if (env__WEBPACK_IMPORTED_MODULE_6__.name === "development") {} // config.openInEditor()
  // config.clear()
  // Tray


  const trayImage = electron__WEBPACK_IMPORTED_MODULE_2__["nativeImage"].createFromPath(`${__dirname}/assets/images/trayTemplate.png`);
  tray = new electron__WEBPACK_IMPORTED_MODULE_2__["Tray"](trayImage);
  tray.on('click', function () {
    if (mainWindow !== undefined) toggleWindow();
  });
  authWindow = Object(_helpers_window__WEBPACK_IMPORTED_MODULE_5__["default"])('auth', {
    backgroundColor: '#d2f1f4',
    // sea green
    width: 750,
    height: 620,
    center: true,
    transparent: true,
    resizable: env__WEBPACK_IMPORTED_MODULE_6__.name === "development",
...
const createMainWindow = () => {
  ...
  if (env__WEBPACK_IMPORTED_MODULE_6__.name === "development") {// mainWindow.openDevTools({mode: 'detach'});
  }
...
const showUserAuthWindow = () => {
  authWindow.loadURL(url__WEBPACK_IMPORTED_MODULE_1___default.a.format({
    pathname: path__WEBPACK_IMPORTED_MODULE_0___default.a.join(__dirname, "auth.html"),
    protocol: "file:",
    slashes: true
  }));

  if (env__WEBPACK_IMPORTED_MODULE_6__.name === "development") {// authWindow.openDevTools({mode: 'detach'});
  }
...
electron__WEBPACK_IMPORTED_MODULE_2__["app"].on('will-quit', () => {
  electron__WEBPACK_IMPORTED_MODULE_2__["globalShortcut"].unregister('CommandOrControl+Shift+K');
});
electron__WEBPACK_IMPORTED_MODULE_2__["app"].on("window-all-closed", () => {
  electron__WEBPACK_IMPORTED_MODULE_2__["app"].quit();
});
electron__WEBPACK_IMPORTED_MODULE_2__["app"].on('open-url', function (event, url) {
  event.preventDefault();
  authWindow.webContents.send('authorized', url);
});
electron__WEBPACK_IMPORTED_MODULE_2__["ipcMain"].handle('getConfig', (e, key) => {
  if (key === undefined) {
    return config.store;
  } else {
...
function updateConfig(arg) {
  let current = JSON.parse(JSON.stringify(config.get(arg.key)));
...
electron__WEBPACK_IMPORTED_MODULE_2__["ipcMain"].handle('replace-config', (e, arg) => {
  return replaceConfig(arg);
});
electron__WEBPACK_IMPORTED_MODULE_2__["ipcMain"].handle('update-config', (e, arg) => {
  return updateConfig(arg);
});
electron__WEBPACK_IMPORTED_MODULE_2__["ipcMain"].handle('get-setup-state', () => {
  let store = config.store;
  return {
...
electron__WEBPACK_IMPORTED_MODULE_2__["ipcMain"].on('setup-complete', (e, key) => {
  authWindow.hide();
  createMainWindow();
});
...
/* harmony default export */ __webpack_exports__["default"] = ((name, options) => {
  ...
  win = new electron__WEBPACK_IMPORTED_MODULE_0__["BrowserWindow"](Object.assign({}, options)); // win = new BrowserWindow(Object.assign({}, options, state));

  win.on("close", saveState); // win.on("blur", () => {win.hide()})
